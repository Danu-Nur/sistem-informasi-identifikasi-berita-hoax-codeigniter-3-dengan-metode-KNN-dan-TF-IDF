...
// proses preprocessing data teks
	public function preProcessing($teks){
		...
		$astoplist = array(
			...
			'bolehkah','bolehlah','buat','bukankah','bukanlah','bukannya','bulan','bung',
...
// proses TF (Trem Frekuensi) data Training / Dataset
	public function buatIndexDataset()
	{
	...
	foreach($berita as $b){
		...
		$jumlah = array_count_values($arberita);
		foreach($jumlah as $j => $j_value){
			// jika trem tidak kosong
			if($j != ""){
				// cek data
				// $row_count = count($this->M_Kata_Dataset->getAnd($idDoc,$j)->result_array());

				$data = array(
					'ID_BERITA' => $idDoc,
					'TREM_DATASET' => $j,
					'FREK_DATASET' => $j_value
				);
				
				// if($row_count > 0){
				// 	// jika baris lebih dari 0
				// 	// update frek trem tb_bobot_kata_dataset
				// 	$this->M_Kata_Dataset->updateData($data);
				// }else{
					// jika baris = 0
					// buat data baru tb_bobot_kata_dataset
...
public function cekKategori($id)
{
	if($id == null){
		...
		$cekber = $this->M_Uji->getAll();
	}else{
	//  ambil id dari kondisi bobot testing
		$cekber = $this->M_Uji->getById($id);
	}
	foreach ($cekber as $cb) {
		$idber = $cb['ID_DATA_UJI'];
		$key = $this->M_Keyword->getAll();
		foreach ($key as $k) {
			$keyword = $k['KEYWORDS'];
			$arkey = explode(" ",$keyword);
			$searchTerm = array();
			foreach ($arkey as $keyterm) {
				if (!empty($keyterm)) {
					$searchTerm[] = "DATA_UJI LIKE '%$keyterm%'";
				}
			}
			$gabung = implode(' AND ', $searchTerm);
			$data = array(
						'ID_DATA_UJI' => $idber,
						'DATA_UJI' => $gabung,
					);
			
			$result = $this->M_Uji->cekK($data);
			
			if ($result > 0) {
				$this->M_Metode->buatIndexKataUji($idber);

				$this->M_Metode->hitungBobotUji($idber);

				$this->M_Metode->hitungKNN($idber);
				break;
			}else {
				$data2 = array(
					'ID_DATA_UJI' => $idber,
					'LABEL_IDENTIFIKASI' => 'DATASET TIDAK ADA'
				);

				$cek = $this->M_Identifikasi->cekData($data2);
				if($cek > 0){
					$this->M_Identifikasi->updateData($data2);
				}else{
					$this->M_Identifikasi->add($data2);
				}
			}
		}
	}
...
// proses TF (Trem Frekuensi) data Testing
	public function buatIndexKataUji($id)
	{
		...
		foreach($berita as $b){
			...
			$arberita = explode(" ", trim($teksBerita));
			$uji_j = array_count_values($arberita);
			foreach($uji_j as $u => $u_value){
				if($u != ""){
					// $row =  $this->M_Kata_Uji->getAnd($idDoc,$u);
...
// proses mengurutkan data dari yang terkecil sampai ke besar dan di ambil jumlah K=5
	...
	public function knnClass()
	{
		...
		foreach ($detail as $det) {
			...
			$labH = $this->M_Identifikasi_detail->getById($id);
			...
			$data = array(
				'ID_DATA_UJI' => $id,
				'LABEL_IDENTIFIKASI' => $labH['STATUS_BERITA']
			);
